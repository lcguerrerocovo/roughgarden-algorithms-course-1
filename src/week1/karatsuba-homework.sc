...
def karatsuba(x: List[Int], y: List[Int]): List[Int] = {
  def sumWithCarry(first: List[Int], second: List[Int]): List[Int] = {
    def carry(first: List[Int], second: List[Int], crr: Int): List[Int] = (first,second) match {
      case (List(),List()) => if(crr != 0) crr :: Nil else Nil
      case (x :: xs,y :: ys) => {
        if(x + y + crr < 10) (x + y + crr) :: carry(xs,ys,0)
        else (x + y + crr - 10) :: carry(xs,ys,1)
      }
      case (x :: xs,List()) => {
        if (x + crr < 10) {
          (x + crr) :: carry(xs, List(), 0)
        } else (x + crr - 10) :: carry(xs, List(), 1)
      }
      case (List(),y :: ys) => {
        if (y + crr < 10) {
          (y + crr) :: carry(List(), ys, 0)
        } else (y + crr - 10) :: carry(List(), ys, 1)
      }
    }
    carry(first.reverse,second.reverse,0).reverse
  }
...
assert((karatsuba(toBigIntegerList("5678"),toBigIntegerList("1234")) mkString "") == "7006652")
assert((karatsuba(toBigIntegerList("3141592653589793238462643383279502884197169399375105820974944592"),
  toBigIntegerList("2718281828459045235360287471352662497757247093699959574966967627")) mkString "") == "8539734222673567065463550869546574495034888535765114961879601127067743044893204848617875072216249073013374895871952806582723184")
